...
(0, dotenv_1.config)();
const server = process.env.NODE_BACKEND_DEV || "https://wyvatecustomerappbackendinnode-thdu.onrender.com";
async function getVendor({ latitude, longitude }) {
    try {
        console.log(longitude, latitude);
        const apiUrl = `${server}/vendor/vendorexplore?latitude=${latitude}&longitude=${longitude}&day=${new Date().getDay()}`;
        console.log({ server, longitude, latitude });
